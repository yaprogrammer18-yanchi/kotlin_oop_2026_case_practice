...
- `Phase` – абстрактный базовый класс. Определяет контракт: `communicate()` (обработка ввода), `checkValidation()` (проверка правил), `toLogString()` (лог хода).
- `Dialog` – менеджер фаз. Хранит текущую `Phase`, делегирует ей ввод игрока и возвращает `DialogState` для Presenter'а.
...
#### Presenter (`GameViewModel` + `GameViewListener`)
Координатор приложения. Связывает UI и модель.
- `GameViewListener` – интерфейс колбэков от UI.
- `GameViewModel` – реализует слушатель, управляет жизненным циклом `Game` и `Dialog`, принимает решения (`handleDialogResult`), маршрутизирует ходы (`nextTurn`, `runFinalRound`), обрабатывает условия победы.
